...
export class AuthService {
  ...
  async signIn(dto: SignInDto, res: Response): Promise<ATokenWithUse> {
    ...
    if (user.userAgent !== 'regular')
      throw new UnauthorizedException('Korisnik je registriran preko Googla');
    ...
    if (!user.isVerified)
      throw new UnauthorizedException('Korisnik nije verificiran');
    ...
    return { userId: user.id, token: tokens.access_token };
  }
  ...
  async verifyUser(id: string): Promise<{ message: string }> {
    ...
    if (!verifyToken)
      throw new ForbiddenException('Verificirani token ne postoji');
  ...
  async googleLogin(res: Response, req: Request): Promise<ATokenWithUse> {
